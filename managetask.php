...
if ($action == "addform") {
    ...
    if ($tid) {
            ...
            foreach($assigned as $member) {
                $assignChk = $task->assign($tid, $member);
                if ($assignChk) {
                    if ($settings["mailnotify"]) {
                        ...
                        if (!empty($user["email"])) {
                            ...
                            $themail->send_mail($user["email"], $langfile["taskassignedsubject"] , $langfile["hello"] . ",<br /><br/>" . $langfile["taskassignedtext"] . " <a href = \"" . $url . "managetask.php?action=showtask&id=$id&tid=$tid\">$title</a><br /><br />$text");
                        }
                    }
                }
            }
        ...
        if ($settings["mailnotify"]) {
            foreach($assigned as $member) {
                ...
                if (!empty($user["email"]) && $userid != $user["ID"]) {
                    // send email
                    $themail = new emailer($settings);
                    $themail->send_mail($user["email"], $langfile["taskassignedsubject"] , $langfile["hello"] . ",<br /><br />" . $langfile["taskassignedtext"] . " <a href = \"" . $url . "managetask.php?action=showtask&id=$id&tid=$tid\">$title</a><br /><br />$text<br />");
                }
            }
        }*/
                ...
                if ($assignChk) {
                    if ($settings["mailnotify"]) {
                        ...
                        if (!empty($user["email"])) {
                            // send email
                            $themail = new emailer($settings);
                            $themail->send_mail($user["email"], $langfile["taskupdatedsubject"] , $langfile["hello"] . ",<br /><br/>" . $langfile["taskupdatedtext"] . " <a href = \"" . $url . "managetask.php?action=showtask&id=$id&tid=$tid\">$title</a><br /><br />$text");
                        }
                    }
                }
            }
